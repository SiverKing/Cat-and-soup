...
log(f"缩放后的屏幕分辨率：{width}x{height}")
...
def find_template_in_region(region, template_path, name, threshold=0.58):
    # time.sleep(0.1) # 防止多次同时调用函数高占用CPU
    ...
    # 打印当前匹配相似度
    log(f"{name} 当前匹配相似度: {max_val:.2f}  参考值:{threshold}")
    # threshold = 0.58
...
def lian_dian():
    log("(程序可以通过空格进行暂停，请不要跟程序抢鼠标,若按下空格后请等待出现“暂停中...”)\n正在自动收菜，升级设施，升级食谱...")
    i = 10
    while i != 0:
        i = i-1
        click(242, 991) # 收菜
...
def main():
    ...
    YiDong_sta(2)
    global Key_thread
    Key_thread = threading.Thread(target=key_task) # 创建线程
    Key_thread.start() # 启动线程
    time.sleep(2)
    ...
    x_path = ".\\img\\x.png"
    xx_path = ".\\img\\xx.png"
    xxx_path = ".\\img\\xxx.png"
    x4_path = ".\\img\\xxxx.png"
    # 定义要点击的点 (x, y) (518, 107), 
    if server == 1:
        # 官服
        log("\n使用官服参数\n")
        click_points = [(518, 107), (248, 986), (241, 340), (480, 533), (430, 597), (551, 592)]  # 示例值 蜗牛
        qw_points = [(518, 107), (248, 986), (241, 340), (480, 533), (430, 597), (551, 592)]  # 青蛙
        hd_points = [(518, 107), (248, 986), (241, 340), (480, 533), (430, 597)]  # 蝴蝶
    elif server == 2:
        # 小米
        log("\n使用小米服参数\n")
        click_points = [(518, 107), (248, 986), (241, 275), (480, 533), (430, 597), (551, 592)]  # 示例值 蜗牛
        qw_points = [(518, 107), (248, 986), (241, 275), (480, 533), (430, 597), (551, 592)]  # 青蛙
    ...
    jl_points = [(562, 585)]
    x_points = [(723, 72)]  # xxx  x4:733 74

    init_roll() # 初始化缩放猫猫界面到最大
    ...
    while True:
        if stop_sta():
            ...
            i=0
            if find_template_in_region(region, x_path, '关闭广告', 0.75) \
            or find_template_in_region(region, xx_path, '关闭广告', 0.70) \
            or find_template_in_region(region, xxx_path, '关闭广告', 0.75) :
                log('关闭广告匹配成功！！！')
                time.sleep(3)
                ...
                or find_template_in_region(region, xxx_path, '关闭广告', 0.75) :
                    log('触发二次关闭！！')
                    perform_click_actions(x_points) # 触发二次关闭
                    time.sleep(1)
            elif find_template_in_region(region, x4_path, '关闭广告', 0.6): # 恶心X广告特殊处理
                log('x4关闭广告匹配成功！！！')
                time.sleep(3)
                click(733, 74)
                time.sleep(1)
            ...
            err = err - 1
            log(f"礼物、广告纠错检查倒计时计数: {err}")
            if err <= 1:
                if find_template_in_region(region, lw_path, '礼物纠错'):
                    log('礼物纠错匹配成功')
                    perform_click_actions(lw_points)
                if find_template_in_region(region, jl_path, '广告纠错'):
                    log('广告纠错匹配成功')
                    perform_click_actions(jl_points)
                    time.sleep(10)
                err = 20
